...
def wavelet_f(nsamples, wtype='db4'):
    """
    Returns wavelet transform matrix in csr format
...
def wavelet_i(nsamples, wtype='db4'):
    """
    Returns inverse wavelet transform matrix in csr format
    ...
    def __init__(self, ckt, nsamples, T, wavelet, multilevel, deriv, sstep):
        ...
        if wavelet == 'none':
            ...
            if multilevel:
                ...
                self.W = wavelet_f(self.nsamples, wavelet)
                self.Wi = wavelet_i(self.nsamples, wavelet)
            else:
