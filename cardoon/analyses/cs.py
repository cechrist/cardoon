"""
:mod:`cs` -- Compressed sensing steady-state analysis
-------------------------------------------------------------------
...
class CS(ParamSet):
    ...
    paramDict = dict(
        ...
        speed = ('Speed of the Algorithm', '', float, 1.0),
	...
	cs = nodalCS.CSNodal(circuit, self.nsamples, self.T, self.m, self.speed)
        if (self.dcguess):
	...
	sV = cs.get_source()
        ...
        xMatrix = xHat*0
        for i in range(cs.xMatrix.shape[0]):
            xMatrix = np.vstack((xMatrix,np.reshape(cs.xMatrix[i,:][p], (circuit.nD_dimension, self.nsamples))[1,:]))
...
#        import pdb; pdb.set_trace()
        np.savetxt('csoutputs.txt', xMatrix, delimiter=',')
