"""
:mod:`nodal` -- Nodal Analysis using WD 
-----------------------------------------

.. module:: nodalWD
.. moduleauthor:: Carlos Christoffersen, Kris Fedick

This module contains specific classes/functions for WD nodal
analysis. 
...
class WDNodal(nd._NLFunctionSP):
    """
    Builds WD system of equations

    This class only sets up WD equations. Equations are solved
    elsewhere. 
    ...
    def __init__(self, ckt, nsamples, T, m):
        """
        Arguments:
        ...
        self.deltaxVec = np.zeros(self.dim)
...
#        import pdb; pdb.set_trace()
	...
	# Update the conductance/current jacobian
        ...
        self.DWM = self.D*self.WM

        # Generate WM dot CS inverse. 
        self.WMinv = scipy.linalg.pinv2(self.WM.todense())
        
        # D = sp.dia_matrix((data,offsets), shape=(self.nsamples, self.nsamples))
        # Id = sp.identity(self.ckt.nD_dimension, format='dia')
        # BDcoo = sp.kron(Id, D).tocoo()
        
        self.IkWM = sp.kron(Id, self.WM).tocoo()*self.p
        self.IkWMinv = self.pt*sp.kron(Id, self.WMinv).tocoo()
        
        # G kron WM and I kron WM
        self.GkWM = sp.kron(self.G, self.WM).tocoo()
    ...
    def get_source(self):
        ...
        for j, ctime in enumerate(self.timeVec):
            ...
            for elem in self.ckt.nD_sourceTDElem:
                ...
                if outTerm[1] >= 0:
                    self.sVecuncomp[outTerm[1]+offset] += current
            self.sVec = self.IkWM*self.sVecuncomp
    ...
    def get_i(self, xVec):
        ...
        self.iVecUC = self.IkWMinv*self.iVec  
        ...
        self.iVec = self.IkWM*(self.iVecUC + self.BD * self.qVec)
        return self.iVec
    ...
    def get_i_Jac(self, xVec):
        ...
        self.iVec = self.IkWM*(self.iVecUC+self.BD *self.qVec)
        #self.iVec = csf.SparsifyWM(self.iVec, self.wmsparsity, self.ckt.nD_dimension)
        
        #import pdb; pdb.set_trace()
        # print(self.iVec)
        BC = self.BCcoo.tocsc()
        #import pdb; pdb.set_trace()
        Jac = self.IkWM*(self.BGcoo.tocsc() + self.BD * BC)*self.IkWMinv
        #import pdb; pdb.set_trace()
