...
def CSMatrix_Inverse (u, s, v, Vect = 0, Type = 'Quick'):
    ...
    if (Type == 'Pinv'):
        ...
        #  pseudo inverse.
        r = sparse.dia_matrix((1/s, 0), shape = (u.shape[0], v.shape[1])).todense()
        PSIinv = np.dot(r.transpose(), np.transpose(u))
        PSIinv = np.dot(np.transpose(v), PSIinv)
        
    else:
        
        r = np.eye(u.shape[0], v.shape[1])
        r = sparse.dia_matrix((1/s, 0), shape = (u.shape[0], v.shape[1])).todense()    
        PSIinv = np.dot(np.transpose(u), Vect.A1)
        PSIinv = np.dot(np.transpose(r), PSIinv.A1)
        PSIinv = np.dot(np.transpose(v),PSIinv.A1)
...
def SparsifyWM (x, m, numnodes):
    
    # Reject lower x% of non-zero samples
    maxcoeff = int(numnodes*m*np.count_nonzero(x))
    floorcoeff = np.zeros(x.shape[0])
    
    sortedaxis = np.argsort(abs(x))
    startindex = sortedaxis.size-1       
    for j in range (0, maxcoeff):
        floorcoeff[sortedaxis[startindex-j]] = x[sortedaxis[startindex-j]]
    return floorcoeff

def compress (x, sparsity, WM, PSI, numnodes):
    return PSI*SparsifyWM(WM*x, sparsity, numnodes)
    
def SparsifyWDomain (coeff, x, numnodes, nsamples):
    
    # Reject lower x% of non-zero samples
    maxcoeff = int(round(numnodes*x*nsamples))
    floorcoeff = np.zeros(coeff.shape[0])
    
    sortedaxis = np.argsort(abs(coeff))
    startindex = sortedaxis.size-1       
    for j in range (0, maxcoeff):
        floorcoeff[sortedaxis[startindex-j]] = coeff[sortedaxis[startindex-j]]
    return floorcoeff
    
    
def CSGetDeltaX (xprev, Coeff, Numnodes, CSMatrix, Jac, WM, Convergence, UseQuickAlgorithm = 'True', SearchSpeed = 0):
    ...
    ``Jac`` : The Jacobean of the system.
    
    ``WM`` : The Wavelet Matrix.
    
    ``Convergence`` :  The Convergence Criterion (DeltaXlastiteration-DeltaXthisiteration < Convergence)
    ...
    SupportMatrix = np.zeros([sparsity*Numnodes,numcoeff*Numnodes])
    RecoveryMatrix = np.kron(np.eye(Numnodes), CSMatrix)
    AttemptMatrix = np.empty((CSMatrix.shape[0], CSMatrix.shape[0]))
    F = np.zeros([sparsity*Numnodes,numcoeff*Numnodes])
    g = np.empty([numcoeff*Numnodes])
    colnorms = np.empty(numcoeff*Numnodes)
    j = 0
    ...
    SearchSpeed = 0
    DeltaX = 1
    testcoeff = sparse.linalg.spsolve(Jac, Coeff)
    xvec = xprev*0
    xold = xprev*0
    newrow = Coeff
    while (DeltaX>Convergence):
    
        g = abs(np.dot(np.transpose(RecoveryMatrix), newrow))
        ...
        if (SearchSpeed==0):
            ...
            for x in range(0, Numnodes):
                sort = np.argsort(g[numcoeff*x:numcoeff*x+numcoeff])
                for y in range(0, int(SearchSpeed*(numcoeff-Totalcols))):
                    j = sort[numcoeff-1-y]
                    Support[numcoeff*x+j] += 1
                    # Add new supported column to AI
                    SupportMatrix[:,numcoeff*x+j]=np.transpose(RecoveryMatrix[:,numcoeff*x+j])
        ...
        # Calculate new adjustment to X and add the adjustment to it
        if (UseQuickAlgorithm == True):
            AttemptMatrix = np.dot(SupportMatrix, WM)
            u,s,v = np.linalg.svd(AttemptMatrix, full_matrices = 1)
            F[:sparsity*Numnodes, :sparsity*Numnodes] = np.diag(1/s)
            AL = np.dot(Jac, np.transpose(v))
            AL = np.dot(AL, np.transpose(F))
            AL = np.dot(AL, np.transpose(u))
            AttemptMatrix = sparse.csc_matrix(np.dot(SupportMatrix, Jac), AL)
            xvec = sparse.linalg.spsolve(AttemptMatrix, Coeff)
        else:
            
            
            AttemptMatrix = np.dot(SupportMatrix, WM)
            xvec = np.dot(np.linalg.pinv(AttemptMatrix), testcoeff).A1
            
            #xvec = np.dot(np.linalg.pinv(AttemptMatrix), Coeff).A1#sparse.linalg.spsolve(AttemptMatrix, Coeff)
        ...
        xold = xvec
        newrow = np.dot(RecoveryMatrix, np.dot(WM, xprev - xvec).A1)
