"""
:mod:`nodalSparseWavelet` -- Steady-state Sparse Nodal Analysis Using Wavelets
-----------------------------------------------------------------
...
class SparseWaveletNodal(nd._NLFunctionSP):
    """
    ...
    def __init__(self, ckt, nsamples, T, wavelet, multilevel, deriv, sstep, method,
                 matformat='csr'):
        """
        Arguments:
        ...
        """
        # Init base class
        super(SparseWaveletNodal, self).__init__()
        # Source stepping factor
        self._sstep = sstep
        # Override convergence helpers (generic gmin does not work with this)
        self.convergence_helpers = [self.solve_simple_sparse, 
                                    self.solve_homotopy_source_sparse,
                                    self.solve_homotopy_source2_sparse, 
                                    None]
    ...
    #----------------------------------------------------------------------
    # Convergence helpers added for nonlinear system solution
    #----------------------------------------------------------------------
