"""
:mod:`nodal` -- Nodal Analysis using CS 
-----------------------------------------

.. module:: nodalCS
.. moduleauthor:: Carlos Christoffersen, Kris Fedick

This module contains specific classes/functions for CS nodal
analysis. 
...
from fsolve import csfsolve_Newton, NoConvergenceError
...
class CSNodal(nd._NLFunctionSP):
    """
    Builds CS system of equations

    This class only sets up CS equations. Equations are solved
    elsewhere. 
    ...
    def __init__(self, ckt, nsamples, T, m, speed):
        """
        Arguments:
        ...
        """
        # Init base class
        super(CSNodal, self).__init__()
        # Override convergence helpers (generic gmin does not work with this)
        self.convergence_helpers = [self.CSsolve_simple, 
                                    None]
        ...
        self.T = T
        self.wmsparsity = m
        self.m = int(np.round(2*m*self.nsamples))
        self.speed = speed
        self.csdim = self.ckt.nD_dimension*self.m
        # Allocate matrices/vectors
        # G and C (done in refresh())

        # iVec = G x + i(x)   total current
        self.iVec = np.zeros(self.csdim)
        self.iVecUC = np.zeros(self.dim)
        # Total charge: C x + q(x)
        self.qVec = np.zeros(self.dim)
        # Source vector 
        self.sVec = np.zeros(self.csdim)
        self.sVecuncomp = np.zeros(self.dim)
        # Delta_x includes samples for all nodes in circuit
        self.deltaxVec = np.zeros(self.csdim)        
        
        self.prevx = np.zeros(self.dim)
...
#                            elem.nD_fpos, elem.nD_fneg, elem.get_G_matrix())
        ...
        self.D = sp.dia_matrix((data,offsets), shape=(self.nsamples, self.nsamples))
        Id = sp.identity(self.ckt.nD_dimension, format='dia')
        BDcoo = sp.kron(Id, self.D).tocoo()
#        import pdb; pdb.set_trace()
        # Pre-multiply by P.T
	...
	# Update the conductance/current jacobian
        ...
        self.WM = csf.wavelet_matrix(self.nsamples)
        self.WMinv = sp.coo_matrix(self.WM.transpose())
        
        # Generate CS matrix.
        self.PSI = csf.cs_matrix(self.m,self.nsamples)
        self.PSIinv = sp.coo_matrix(scipy.linalg.pinv2(self.PSI))
        
        # Generate PSI dot WM and PSI dot DWM
        self.PSIWM = self.PSI*self.WM
        self.PSIDWM = self.PSI*self.D*self.WM
        
        # Generate WM dot CS inverse. 
        #self.u,self.s,self.v = np.linalg.svd(self.PSIWM, full_matrices = 1)
        #self.PSIWMinv = csf.CSMatrix_Inverse (self.u, self.s, self.v, Type = 'Pinv')
        self.PSIWMinv = scipy.linalg.pinv2(self.PSIWM)
        
        # D = sp.dia_matrix((data,offsets), shape=(self.nsamples, self.nsamples))
        # Id = sp.identity(self.ckt.nD_dimension, format='dia')
        # BDcoo = sp.kron(Id, D).tocoo()

        self.p = sp.coo_matrix(np.eye(self.dim))
        self.p.col = P
        self.pt = self.p.transpose()
        
        self.IkWM = sp.kron(Id, self.WM).tocoo()
        self.IkPSI = sp.kron(Id, self.PSI).tocoo()
        self.IkPSIinv = sp.kron(Id, self.PSIinv).tocoo()
        self.IkWMinv = sp.kron(Id, self.WMinv).tocoo()
        
        # G kron WM and I kron WM
        self.GkPSIWM = sp.kron(self.G, self.PSIWM).tocoo()
        self.IkPSIWM = sp.kron(Id, self.PSIWM).tocoo()*self.p
        
        # C kron DWM and I kron DWM
        self.CkPSIDWM = sp.kron(self.G, self.PSIDWM).tocoo()
        self.IkPSIDWM = sp.kron(Id, self.PSIDWM).tocoo()
        
        # I kron PWIWMinv
        self.IkPSIWMinv = self.pt*sp.kron(Id, self.PSIWMinv).tocoo()
    ...
    def get_source(self):
        ...
        for j, ctime in enumerate(self.timeVec):
            ...
            for elem in self.ckt.nD_sourceDCElem:
                ...
                # import pdb; pdb.set_trace()
                if outTerm[0] >= 0:
                    self.sVecuncomp[outTerm[0]+offset] -= current
                if outTerm[1] >= 0:
                    self.sVecuncomp[outTerm[1]+offset] += current
            for elem in self.ckt.nD_sourceTDElem:
                ...
                if outTerm[1] >= 0:
                    self.sVecuncomp[outTerm[1]+offset] += current
            self.sVec = self.IkWM*np.dot(self.p.todense(), self.sVecuncomp).A1
            self.sVec = csf.SparsifyWM(self.sVec, self.wmsparsity, self.ckt.nD_dimension)
            self.sVec = self.IkPSI*self.sVec
    ...
    def get_i(self, xVec):
        ...
        iVecUC: uncompressed output vector of currents (for all samples)
        """

        self.iVecUC = self.IkPSIWMinv*self.iVec
        
        
        for j in range(self.nsamples):
            base = self.ckt.nD_dimension * j
            itmp = self.iVecUC[base:base+self.ckt.nD_dimension]
        ...
        self.iVec = self.IkPSIWM*(self.iVecUC + self.BD * self.qVec)
        return self.iVec
    ...
    def get_i_Jac(self, xVec):
        ...
        Jac: system Jacobian
        """
        #xVec = self.IkPSIWMinv*xVec
        
        
        self.storedata(xVec)
        #xVec, SupportMatrix = csf.CSRecover (xVec, self.ckt.nD_dimension, self.PSIWM, True, SearchSpeed = 1)
        #import pdb; pdb.set_trace()        
        #xVec = self.IkWMinv*xVec        
        self.BGcoo._base_idx = self.baseG
        self.BCcoo._base_idx = self.baseC
        #print(max(abs(xVec)))
        #import pdb; pdb.set_trace()
        # Linear contribution
        #self.iVecUC = self.BGcoo * xVec
        #self.qVec = self.BCcoo * xVec
        ...
        # iVec = [GkPSIWM+CkPSIDWM]x + IkPSIWMi(x) + IkPSIDWMq(x)
        ...
        BC = self.BCcoo.tocsc()
        #import pdb; pdb.set_trace()
        Jac =  self.IkPSIWM*(self.BGcoo.tocsc() + self.BD * BC)*self.IkPSIWMinv
        #import pdb; pdb.set_trace()
